...
#define  ASSR_CONTROL				0x03
#define  ASSR_STANDARD				BIT(0)
#define  ASSR_ALTERNATIVE			BIT(1)
				       ...
				       struct drm_bridge_state *old_bridge_state)
{
	 ...
	 * we need to disable the scrambler and use the default DP scrambler seed.
	 */
	force_dp = of_property_read_bool(pdata->dev->of_node, "force-displayport");
	if (force_dp || pdata->bridge.type == DRM_MODE_CONNECTOR_DisplayPort) {
		/* This field is only R/W when TEST2 pin is high */
		regmap_update_bits(pdata->regmap, SN_ENH_FRAME_REG,
				   ASSR_CONTROL, ASSR_STANDARD);
